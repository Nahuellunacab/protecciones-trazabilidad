...
- El dashboard incluye un **resumen ejecutivo generado por IA** (`GET /api/dashboard/resumen-ia`): `DashboardService.obtenerResumenIA()` arma un prompt en texto plano con los KPIs generales y las distribuciones (estado, marca, destino, proveedor) y se lo pasa a `LLMService.generarTexto`, pidiendo la respuesta en un formato fijo (encabezado + 3-5 viñetas) que el frontend parsea para mostrarlo con estructura visual. **Ya no se pide automáticamente al cargar el dashboard**: el frontend (`HomePage.tsx`) solo llama al endpoint cuando el usuario aprieta el botón "Generar resumen"/"Actualizar", para no consumir cuota de Gemini en cada visita a la página. Es una funcionalidad **opcional y degradable**: si no hay `GEMINI_API_KEY` configurada o la llamada falla (timeout, 429 en todas las claves, etc.), el endpoint devuelve `resumen: null` y el frontend muestra un mensaje en vez de romper. El resultado se cachea en memoria por 4 horas fijas (`RESUMEN_IA_TTL_MS`, ya no se invalida por cambios en los KPIs). El endpoint acepta `?forzar=true` (usado por ese mismo botón en cada click, incluida la primera generación) para saltear el cache y regenerar el resumen bajo demanda; el acceso al cache/regeneración está sincronizado en `DashboardService` para que pedidos concurrentes no disparen más de un llamado a Gemini a la vez.
...
Migraciones SQL versionadas en `backend/src/main/resources/db/migration/V{n}__descripcion.sql` (Flyway). Al momento de este análisis van de V1 a V29:
