...
- CORS permite por defecto cualquier puerto de `http://localhost` y `http://127.0.0.1` (`allowedOriginPatterns` + `allowCredentials(true)` en `CorsConfig.java`), para que funcione tanto `npm run dev` (5173) como el build dockerizado (5173 vía Nginx) sin tener que hardcodear un puerto. La lista de orígenes es parametrizable vía `cors.allowed-origins` (`CORS_ALLOWED_ORIGINS`, separada por coma) para poder agregar el dominio real en un despliegue de producción sin tocar código.
- El login (`POST /api/auth/login`) tiene protección contra fuerza bruta: `LoginRateLimiter` (`protecciones.security`) cuenta intentos fallidos en memoria por identificador **y** por IP de origen, y bloquea temporalmente (`auth.login.max-intentos` fallos, default 5, por `auth.login.bloqueo-minutos`, default 15) devolviendo HTTP 429 (`TooManyRequestsException`). Es en memoria porque hoy corre una sola instancia de backend; si se escala horizontalmente habría que moverlo a un store compartido.
...
- Las funcionalidades de IA usan una abstracción **`LLMService`** (interfaz en `protecciones.service.llm`, método principal `generarTexto(systemPrompt, userPrompt, maxOutputTokens)` + `estaDisponible()`), implementada hoy por **`GeminiService`** contra la **Gemini API** (Google AI Studio, capa gratuita — elegida por eso, dado el volumen bajo de uso esperado) vía `RestClient`. `DashboardService` y `CopilotoIAService` dependen de la interfaz `LLMService`, no de `GeminiService` directamente. La excepción es `RemitoIAService` (carga inteligente de relés, ver más abajo), que necesita mandarle un archivo a Gemini y por eso depende de `GeminiService` concreto, para usar su método extra `generarTextoConArchivo(...)` que no forma parte de la interfaz. Importante: Gemini 2.5 Flash tiene "thinking" (razonamiento interno) activado por defecto, que puede consumir casi todo `maxOutputTokens` y truncar la respuesta (`finishReason: MAX_TOKENS`) — por eso `GeminiService` fuerza `generationConfig.thinkingConfig.thinkingBudget: 0` en cada request.
- **`GEMINI_API_KEY` admite varias claves separadas por coma** (una por cuenta de Google AI Studio, ej. `clave1,clave2,clave3`), para repartir el uso entre varias cuotas gratuitas. `GeminiService` mantiene un índice en memoria (`AtomicInteger`) y rota automáticamente a la siguiente clave cuando la actual devuelve 429 (cuota agotada); el puntero es "pegajoso" (sticky): no vuelve a la primera clave por su cuenta, solo avanza ante un nuevo 429. Si todas las claves devuelven 429, se propaga el error (y el feature se degrada como cualquier otro fallo de Gemini).
- El dashboard incluye un **resumen ejecutivo generado por IA** (`GET /api/dashboard/resumen-ia`): `DashboardService.obtenerResumenIA()` arma un prompt en texto plano con los KPIs generales y las distribuciones (estado, marca, destino, proveedor) y se lo pasa a `LLMService.generarTexto`, pidiendo la respuesta en un formato fijo (encabezado + 3-5 viñetas) que el frontend parsea para mostrarlo con estructura visual. El frontend (`HomePage.tsx`) **lo pide automáticamente al montar el dashboard**, pero sin forzar (`?forzar` ausente): como el backend cachea la respuesta 4 horas, esto en la práctica solo le pega a Gemini cuando el cache venció o nunca se generó, sin gastar cuota en cada visita a la página. El botón "Actualizar" sigue forzando una regeneración inmediata salteando ese cache (`?forzar=true`). Es una funcionalidad **opcional y degradable**: si no hay `GEMINI_API_KEY` configurada o la llamada falla (timeout, 429 en todas las claves, etc.), el endpoint devuelve `resumen: null` y el frontend muestra un mensaje en vez de romper. El resultado se cachea en memoria por 4 horas fijas (`RESUMEN_IA_TTL_MS`, ya no se invalida por cambios en los KPIs); `ResumenIADTO` incluye además `generadoEn` (el momento real en que se generó el resumen, no el momento en que respondió el endpoint), que el frontend usa para mostrar la hora correcta aunque la respuesta venga de cache. El acceso al cache/regeneración está sincronizado en `DashboardService` para que pedidos concurrentes no disparen más de un llamado a Gemini a la vez.
- **Copiloto IA** (`POST /api/copiloto/consultar`, `CopilotoController` → `CopilotoIAService`): chat del dashboard que arma contexto (KPIs, distribuciones, últimos movimientos, vía `DashboardService`) y se lo manda a `LLMService` junto con la consulta del usuario. La respuesta del modelo puede ser texto libre (`tipo: "RESPUESTA"`, markdown, renderizado en el frontend con `react-markdown` + `remark-gfm`) o una **acción de navegación/filtrado** (`tipo: "ACCION"`) validada contra una whitelist fija: `FILTRAR_RELES` (marca/modelo/estado/proveedor/destino), `ABRIR_RELE` (por número de serie), `IR_A_MODULO` (DASHBOARD/RELES/MOVIMIENTOS/ADMINISTRACION). Cualquier acción fuera de esa whitelist se degrada a respuesta de texto. El Copiloto **nunca ejecuta escrituras** (altas/bajas/movimientos), solo lee y navega. Componente frontend: `components/dashboard/CopilotoIACard.tsx`.
- **Carga inteligente de relés por remito** (`POST /api/remitos/analizar` con el PDF/imagen adjunto, `POST /api/remitos/analizar/revalidar` con datos corregidos a mano): `RemitoIAService` le manda el archivo a Gemini (multimodal, vía `GeminiService.generarTextoConArchivo`) pidiéndole que extraiga número de remito, fecha, proveedor, orden de provisión y la lista de relés (marca/modelo/código de configuración/número de serie), excluyendo explícitamente accesorios (zócalos, test blocks, etc.). El resultado se valida localmente contra los catálogos existentes (`Marca`, `Modelo`, `Proveedor`, `Rele`) — incluye relleno de campos faltantes por agrupación de series, matching de modelo tolerante a errores de OCR (Levenshtein) y detección de números de serie duplicados o ya existentes. El endpoint devuelve una **propuesta** (`RemitoAnalisisResponseDTO`), nunca guarda nada por sí solo: el alta real de cada relé sigue pasando por el flujo normal de `POST /api/reles`. Componente frontend: `components/rele/CargaInteligenteRemitoDialog.tsx`, disparado desde `ReleForm.tsx`.
...
- **OpenPDF (`com.github.librepdf:openpdf`)** — generación de PDFs: export del dashboard a PDF y conversión de fotos (JPEG/PNG/WebP) a PDF de una página en `ArchivoAdjuntoValidator` (ver Reglas del proyecto, punto 13).
...
├── controller/                      # capa HTTP (@RestController), un archivo por recurso (incluye AuthController, CopilotoController)
├── service/                         # lógica de negocio (@Service), un archivo por recurso (incluye AuthService, CopilotoIAService, RemitoIAService)
│   └── llm/                         # LLMService (interfaz) + GeminiService (implementación, Gemini API)
...
Migraciones SQL versionadas en `backend/src/main/resources/db/migration/V{n}__descripcion.sql` (Flyway). Al momento de este análisis van de V1 a V30:
- **V27** agrega autenticación a `usuario` (ver `docs/autenticacion.md`).
- **V28** agrega `usuario.numero_sobre` (legajo, `NOT NULL UNIQUE`) y recompone la `CHECK` de `usuario.rol` sin agregar roles nuevos (sigue siendo `ADMIN`/`OPERADOR`/`AUDITOR`).
- **V29** es una migración de datos única (~3600 líneas), sin cambios de esquema permanente: importa datos operativos reales desde la base Access que este sistema reemplaza (provincias, localidades, destinos, posiciones, proveedores, remitos, relés y su historial de movimientos).
- **V30** agrega un índice **único funcional** case-insensitive sobre `UPPER(TRIM(numero_remito))` en `remito` (`uk_remito_numero_remito`), sin normalizar los valores ya cargados (son textos descriptivos libres, no códigos cortos como `rele.numero_serie`). Cierra la ventana de carrera que dejaba abierta la sola validación en Java (ver Reglas del proyecto, punto 14).
...
│   ├── rele/                # ReleForm, ReleTable, CargaInteligenteRemitoDialog (carga por IA)
│   ├── dashboard/           # CopilotoIACard (chat IA con acciones de navegación/filtrado)
│   └── common/              # componentes genéricos reutilizables (EmptyState, PageHeader, SelectorArchivoAdjunto)
...
- **Movimiento** (`rele_id`, `estado_id`, `posicion_id`, `usuario_id`) — evento histórico e **inmutable** (nunca se actualiza ni se borra, solo se inserta). Es la fuente de verdad de:
  ...
  - En el frontend, `MovimientoForm.tsx` separa la elección en dos pasos (Destino → Posición, selector dependiente que se recarga vía `GET /posiciones/destino/{destinoId}`) para mejorar la UX, pero es puramente visual: el `MovimientoRequest` que se manda al backend sigue llevando solo `posicionId`, nunca `destinoId`.
...
12. Errores de negocio se señalizan con `BusinessException` (→ HTTP 400 vía `GlobalExceptionHandler`), no con excepciones genéricas ni con `null` silencioso. `GlobalExceptionHandler` tiene además un catch-all de `Exception` (→ HTTP 500 genérico, logueado del lado del servidor) para que ningún error no previsto filtre stacktrace ni detalles internos al cliente; `server.error.include-stacktrace`/`include-message` están explícitamente en `never`.
13. Los recursos con archivos adjuntos (`Remito`, `OrdenProvision`) guardan el PDF en el filesystem local (`uploads/remitos`, `uploads/ordenes-provision`) — el nombre en disco siempre es un UUID generado por el backend (nunca el nombre original del archivo, para no arrastrar path traversal), y la ruta se persiste en la entidad; no se guarda el archivo en la base de datos. `ArchivoAdjuntoValidator` (compartido por `RemitoService`/`OrdenProvisionService`) acepta **PDF** (extensión, Content-Type y firma binaria `%PDF`) o una **foto** (JPEG/PNG/WebP, detectada por firma binaria), convirtiendo la foto a un PDF de una página con OpenPDF antes de guardarla — el invariante "el adjunto guardado siempre es un PDF" (storage, Content-Type de descarga) no cambia. El frontend ofrece un botón de cámara (`capture="environment"`, componente `SelectorArchivoAdjunto`) en los selectores de archivo de Remito/OrdenProvision y en la carga inteligente por IA, para sacarle una foto al comprobante en el momento.
14. `remito.numero_remito` se valida como duplicado tanto en `RemitoService` (case-insensitive, en Java, antes del insert) **como** en la base de datos (índice único funcional `uk_remito_numero_remito` sobre `UPPER(TRIM(numero_remito))`, migración V30) — la violación de ese índice se traduce a `BusinessException`, nunca se propaga como `DataIntegrityViolationException` cruda. Además, `RemitoService`/`OrdenProvisionService` borran el PDF anterior del filesystem cuando se reemplaza por uno nuevo o se elimina el registro (fallar ese borrado no rompe la operación principal, solo queda logueado), para no acumular archivos huérfanos en `uploads/`.
...
- **La unicidad de `numeroSerie`.** Cualquier alta/edición de relé debe re-validar duplicados en el Service, no confiar solo en la constraint `UNIQUE` de la tabla (los mensajes de error de negocio deben seguir siendo amigables vía `BusinessException`, no un 500 por `DataIntegrityViolationException` crudo). Mismo criterio para `remito.numero_remito` (índice único funcional `uk_remito_numero_remito`, ver Reglas del proyecto punto 14): la violación del índice se traduce siempre a `BusinessException` en el Service.
